...
## Stale Cache и Offline API

- `stale cache` означает, что `vimit` не смог получить свежий `/v1/me` и показывает последний сохранённый кэш.
- `offline API` означает, что live API сейчас недоступен; если есть cache fallback, в выводе могут одновременно присутствовать оба состояния.
- В JSON это явно отражено через `data_status`, `api_status`, `stale` и `offline_duration_min`.
