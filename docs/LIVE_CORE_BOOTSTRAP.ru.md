...
Также на момент фиксации не существует GitHub-репозитория `xodapi/live-core`.
Каталог `C:\project\live_core` не является git-репозиторием и содержит только
черновые плановые документы. Поэтому первый шаг live-core должен быть
